 ...
 * @param {string} hashingServerIP - If present, will use hashing server
 * @param (number) hashingServeRPort - If present, will use hashing server
        ...
        utils.hashLocation1(this.authTicket, this.lat, this.lng, this.alt).then((hash1) => {
            utils.hashLocation2(this.lat, this.lng, this.alt).then((hash2) => {
                ...
                requests.forEach(function(request) {
                    utils.hashRequest(this.authTicket, request.encode ? request.encode().toBuffer() : request).then((hashRequest) => {
                        ...
                        if(request_hash.length === requests.length) {
                            ...
                            if(retRawBytes){
                                success(signature.encode().toBuffer());
                            } else {
                                success(signature);
                            }
                        };
 ...
 */
Builder.prototype.encrypt = function(requests, timestamp_since_start, cb) {
    let signature = this.buildSignature(requests, true).then(() => {
        utils.encrypt(signature, timestamp_since_start, cb);
    }, () => {
 ...
 */
Builder.prototype.encryptSync = function(requests, timestamp_since_start) {
    const signature = this.buildSignature(requests, true);
    return utils.encryptSync(signature, timestamp_since_start);
}
...
Builder.prototype.getUnknown6 = function(requests, cb) {
    console.warn("WARNING: This is no longer safe as of 0.39 and does not match real requests. Please use hashLocation1, hashLocation2 and hashRequest and fill in the blanks");
