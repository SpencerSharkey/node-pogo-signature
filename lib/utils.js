...
Utils.prototype.encryptTwoFish = function(input, timestamp_since_start, version, cb) {
    let key = [
    ...
    let fish = twofish(null, timestamp_since_start);
    let encrypted = fish.encrypt(key, input);
    encrypted.push(0x21);
    cb(null, encrypted);
};
...
Utils.prototype.encrypt = function(input, timestamp_since_start, version, cb) {
    ...
    let pcryptVersion = version.startsWith('0.59') ? 4 : (version.startsWith('0.45') ? 2 : 3);
