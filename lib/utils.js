...
Utils.prototype.encrypt = function(input, timestamp_since_start, version, cb) {
    ...
    let pcryptVersion = parseFloat(version) > 0.65 ? 5 : parseFloat(version) > 0.59 ? 4 : (version.startsWith('0.45') ? 2 : 3);
 ...
 * @param {*} body 
 */
function retryStrat(err, response, body) {
    ...
    return new Promise((resolve, fail) => {
        ...
        request.post(this.pokeHashUrl, {
            ...
            switch(res.statusCode) {
                ...
                case 400:
                    fail(new errors.HashServerError("A networking error occurred (400)", res.statusCode, data));
                break;
                
                case 429:
                    fail(new errors.HashServerError("A networking error occurred (429)", res.statusCode, data));
                break;
                
                case 401:
                    fail(new errors.HashServerError("A networking error occurred (401)", res.statusCode, data));
                break;
                
                default:
                    fail(new errors.HashServerError("A networking error occurred (500)", res.statusCode, data));
                break;
            }
        })
    });
